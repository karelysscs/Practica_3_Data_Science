"""Cliente de LLM con proveedor intercambiable (OpenAI o Gemini). Ver el
mismo módulo en tarea1_rag_normativo/src/llm_client.py para el razonamiento
completo: el issue no exige un proveedor específico para el LLM que responde
preguntas, así que Gemini (capa gratuita) es una alternativa válida cuando no
hay crédito de OpenAI disponible. No importa UI.
"""
...
def call_llm(provider: str, model: str, system_prompt: str, user_prompt: str, api_key: str) -> LLMResponse:
    if provider == "openai":
